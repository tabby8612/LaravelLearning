...
class User extends Authenticatable
{
    ...
    //? ------------- Section 20 - Some more model manipulation ----------------------------

    // Accessor will help in transforming the represention of the attribute
    // if attribute name is first_name in DB then function name must be in camel case firstName

    // use Illuminate\Database\Eloquent\Casts\Attribute;

    //-- Defining Accessors (get) and Mutators (get)
    protected function name(): Attribute {
        return Attribute::make(
            get: fn(string $value) => strtoupper($value),
            set: fn(string $value) => "$value Sajwani"
        );
    }
